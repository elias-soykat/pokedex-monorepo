...
export function formatStats(stats: TPokemonStat[]): { name: string; value: number; max: number }[] {
  ...
  const totalValue = stats.reduce((total, { base_stat }) => total + base_stat, 0)

  return [...statsObject, { name: 'total', value: totalValue, max: totalValue }]
}
